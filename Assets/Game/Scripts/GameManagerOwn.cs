...
public class GameManagerOwn : MonoBehaviour
{
    public static GameManagerOwn instance { get; set; }
    ...
    private void Start()
    {
        if (PlayerPrefs.GetInt("ObjectTicket") == 1)
        {
            ...
            ObjTicket.transform.GetChild(2).gameObject.SetActive(false);
            ticketCounter++;
        }
        if (PlayerPrefs.GetInt("AlphabetTicket") == 1)
        {
            AlphabetTicket.transform.GetChild(1).gameObject.SetActive(false);
            AlphabetTicket.transform.GetChild(2).gameObject.SetActive(false);
            ticketCounter++;
        }
        if (PlayerPrefs.GetInt("TracingTicket") == 1)
        {
            TracingTicket.transform.GetChild(1).gameObject.SetActive(false);
            TracingTicket.transform.GetChild(2).gameObject.SetActive(false);
            ticketCounter++;
        }
    }
    ...
    public void ObjMatchingAdsWatch()
    {
        AdManager.Instance.ShowInterstitialAd(() => 
        {
            ...
            ObjTicket.transform.GetChild(2).gameObject.SetActive(false);
            ticketCounter++;
            CanvasManager.instance.TicketCollecter();
        });
    }
    public void AlphaMatchingAdsWatch()
    {
        AdManager.Instance.ShowInterstitialAd(() => 
        {
            AlphabetTicket.transform.GetChild(1).gameObject.SetActive(false);
